  ...
  return (
    <details className="job-history-panel auxiliary-panel" aria-label="Read-only job history">
      ...
      <div className="auxiliary-panel-body">
        ...
        {jobs.length === 0 ? <p className="empty-audit">No job history in snapshot.</p> : (
          <ol className="job-history-list">
            {jobs.map((job) => (
              <li key={job.id} className={job.id === active ? 'active' : ''}>
                ...
                <p>{job.request_preview || 'No request preview'}</p>
                {job.final_report_path ? <code>{job.final_report_path}</code> : <small>No final report</small>}
              </li>
            ))}
          </ol>
        )}
